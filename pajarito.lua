...
--- A function that changes level of output to the
--- console, either none (default) or all.
-- @param is_verbose a boolean, if true prints output
-- to the console, flase shows none.
function pajarito.setVerbose(is_verbose)
  if is_verbose then 
    log = print;
  else 
    log = function (...) end
...
-- To work with walls, we need to do bitwise
-- operations in this case just `and` and `or`
...
if type(jit) == 'table' then
    log('Using Lua Jit Bitwise')
    -- import Lua jit bit operations
    bit = require("bit")
    band = bit.band
    bor = bit.bor
elseif v_number >= 5.3 then
    log('Using Lua Built-in Bitwise')
    ...
    if (status) then
        log('Using Lua bit32 Bitwise')
        band = bit.band
        bor = bit.bor
    else
        log('Using Pure Lua Bitwise')
...
function pajarito.addNodeByPriority(node_x, node_y, d, father_index)
    log(' Check node ',node_x,',',node_y,' for father ', pajarito.getNodeOfIndex(father_index))
    ...
    if val == 4 then
        if not isWallBetween(father_index, son_index) then
            ...
            log('    Discarted by walls')
        end
    elseif val == 1 then
        pajarito.markBorderNode(node_x, node_y, pajaritoNode(node_x, node_y, -1, father_index))
        log( 'Error -> ',val, getGridWeight(node_x,node_y))
        log('    marked as -1')
    elseif val == 2 then
        log('     is already on queue')
    elseif val == 3 then
...
function pajarito.getNewFatherNode(father, node_x, node_y)
    log('Check node for best possible father')
...
local function getAndBuildPathInRange(x, y)
    ...
    if not lst_marked_nodes[index] then
        log('Point is not in range',2)
        return false
    end

    --exits the point on the marked ones...
    local new_index_can_exist = true;
    while lst_marked_nodes[index] or not new_index_can_exist do
        local node = lst_marked_nodes[index]
        lst_nodes_on_path[index] = true
        new_index_can_exist = node ~= nil;
        if node then
...
function pajarito.pathfinder(node_x, node_y, dest_x, dest_y)
    ...
    if pajarito.isNodeOnGrid(node_x, node_y) and pajarito.isNodeOnGrid(dest_x, dest_y) then
        queue_of_nodes.push(pajaritoNode(FLOOR(node_x), FLOOR(node_y)))
        lst_nodes_on_queue[getIndexOfNode(node_x, node_y)] = nil
...
function pajarito.addWall(x, y, val)
    ...
    if int and int ~= 0 then
        ...
        if not lst_of_walls[index] then
            ...
            log('Adding wall on ',x,y,'str value:',val,' Bit value:',int)
            return 1
        else
            local merged = bor(lst_of_walls[index], int)
            log('Collision of wall on ',x,y)
            log('Merge old value of',lst_of_walls[ index ],'with',int,'as',merged)
